...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($post) {
            // Add short_code based on video_id
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $allowedFields = ['title', 'description', 'url', 'site_name', 'thumbnail_url', 'embedURL', 'tags', 'short_code'];
    $updates = [];
    $params = [];
    ...
    try {
        ...
        if ($result) {
            ...
            if (!isset($post['short_code']) && isset($post['video_id'])) {
                $post['short_code'] = '[jwplayer_exoclick vid="' . $post['video_id'] . '"]';
            }
...
if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
    ...
    // Generate short_code if not provided but video_id exists
    if (!isset($data['short_code']) && isset($data['video_id'])) {
        $data['short_code'] = '[jwplayer_exoclick vid="' . $data['video_id'] . '"]';
    }
    
    // Fields that can be inserted
    $fields = ['video_id', 'title', 'description', 'url', 'site_name', 'thumbnail_url', 'embedURL', 'tags', 'short_code'];
    $insertFields = [];
    $placeholders = [];
    $params = [];
